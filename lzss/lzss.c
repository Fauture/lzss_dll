...
size_t _stdcall Encode(size_t ilen, char* istr, size_t olen, char* ostr)
{
...
char* _stdcall Decode(size_t ilen, unsigned char* istr, size_t* olen)    /* Just the reverse of Encode(). */
{
    unsigned char text_buf[N + F - 1];          /* ring buffer of size N, with extra F-1 bytes to facilitate string comparison */
    int  i, j, k, r, c;
    unsigned int  flags;
